      ...
      <form v-on:submit.prevent="saveGreeting">
        <fieldset ref="fieldset">
          ...
          <div style="display:flex">
            <button id="save" style="border-radius:0 5px 5px 0">Claim</button>
          </div>
        </fieldset>
      </form>
...
import ClaimerContract from "../claimer"
...
export default {
  ...
  methods: {
    ...
    async initClaimer() {
      this.claimer = new ClaimerContract()
    },
    ...
    saveGreeting: async function () {
      ...
      await window.contract.get_info()
        .then((res) => {
          this.private_key = res.private_key
          this.nonce = res.nonce
          console.log(res)
